...
gnome-terminal --tab --title="piper_control" -- bash -c "cd $PROJECT_ROOT && roslaunch launch/piper_control.launch; exec bash"
sleep 3

# 启动 MoveIt move_group 节点
echo "[3/6] 启动 MoveIt move_group..."
if [ -d "$PROJECT_ROOT/piper_ros/devel" ]; then
    echo "  启动 MoveIt 规划服务..."
    gnome-terminal --tab --title="moveit_group" -- bash -c "source $PROJECT_ROOT/piper_ros/devel/setup.bash && roslaunch piper_with_gripper_moveit demo.launch use_rviz:=false; exec bash"
else
...
gnome-terminal --tab --title="realsense_yolo" -- bash -c "export MPLCONFIGDIR=/tmp/matplotlib-cache && eval \"\$(conda shell.bash hook)\" && conda activate button && cd $PROJECT_ROOT && python3 realsense_yolo_pc_roi.py; exec bash"
sleep 2

# 启动 piper_tf_publisher.py (使用 conda 环境)
echo "[5/6] 启动 piper_tf_publisher.py..."
gnome-terminal --tab --title="piper_tf_publisher" -- bash -c "export MPLCONFIGDIR=/tmp/matplotlib-cache && eval \"\$(conda shell.bash hook)\" && conda activate button && cd $PROJECT_ROOT && python3 piper_tf_publisher.py; exec bash"
sleep 2

# 启动 grasp_action.py (使用系统 Python，启用 MoveIt)
echo "[6/6] 启动 grasp_action.py (MoveIt 模式)..."
gnome-terminal --tab --title="grasp_action_moveit" -- bash -c "source $PROJECT_ROOT/piper_ros/devel/setup.bash && cd $PROJECT_ROOT && python3 grasp_action_backup.py; exec bash"
